...
class TreeLib
{
    ...
    public function arrayTreeByCurrent(array $array, array $available, $parent = null)
    {
        $tree = [];
        foreach ($array as $value) {
            if (in_array($value['id'], $available)) {
                if ($value['parent_id'] == $parent) {
                    $children = $this->arrayTreeByCurrent($array, $available, $value['id']);
                    if ($children) {
                        $value['children'] = $children;
                    }
                    $tree[] = $value;
